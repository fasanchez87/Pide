package com.ingeniapps.pide.sharedPreferences;
...
public class gestionSharedPreferences {
    ...
    public void putHashMapObjectProducto( HashMap<Integer,Producto> getHashMapObjectServicio )
    {
        Gson gson = new Gson();
        String hashMapString = gson.toJson(getHashMapObjectServicio);
        //save in shared prefs
        loginPrefsEditor.putString("HashMapObjectProducto", hashMapString).apply();
    }

    public HashMap<Integer,Producto> getHashMapObjectProducto()
    {
        Gson gson = new Gson();
        boolean failed = false;
        //get from shared prefs
        String storedHashMapString = preferences.getString("HashMapObjectProducto", "oopsDintWork");
        java.lang.reflect.Type type = new TypeToken<HashMap<Integer, Producto>>(){}.getType();

        HashMap<Integer, Producto> testHashMap2 = null;
        try
        {
        ...
        Log.i("entro:shared",""+testHashMap2);

        return testHashMap2;
    }


    public void putListProductos(String key, ArrayList<Producto> productos)
    {
        checkForNullKey(key);
        Gson gson = new Gson();
        ArrayList<String> objStrings = new ArrayList<String>();
    ...
    public ArrayList<Producto> getListProductos(String key, Class<?> mClass){
       Gson gson = new Gson();
       ...
       {
           Object value  = gson.fromJson(jObjString,  mClass);
           objects.add((Producto) value);
       }
       return objects;
   }


    public void putListProductosPedidos(String key, ArrayList<Pedido> pedido)
    {
        checkForNullKey(key);
        Gson gson = new Gson();
        ArrayList<String> objStrings = new ArrayList<String>();
        for(Object obj : pedido)
        {
            objStrings.add(gson.toJson(obj));
        }
        putListString(key, objStrings);
    ...
    public ArrayList<Pedido> getListProductosPedidos(String key, Class<?> mClass){
        Gson gson = new Gson();

        ArrayList<String> objStrings = getListString(key);
        ArrayList<Pedido> objects =  new ArrayList<Pedido>();

        for(String jObjString : objStrings)
        {
            Object value  = gson.fromJson(jObjString,  mClass);
            objects.add((Pedido) value);
        }
        return objects;
    }
